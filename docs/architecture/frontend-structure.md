...
_repositories   = 外部 API（BFF `/api/*`）の adapter
...
_components/app/** (composition root)
  ...
  → _repositories/** (具象 repository の生成・注入は _components/app/repositories.ts で行う)
...
- `AuthContext` — 認証状態と `useAuth()`
- `repositories.ts` — 不変の Repository / Gateway シングルトン（Context に載せない）
- `AppProviders` — 上記 Provider の合成
...
- repository port は、BFF endpoint 都合で横断的になり得るため原則 `_usecases/**/ports.ts` に置く
...
- usecase / query / command は repository 実装を `new` しない（注入は `_components/app/repositories.ts` で行う）
...
- 通知 gateway の抽象と HTTP 実装（`notificationGateway`）

## `_libs`

外部ライブラリの入口・薄い adapter。re-export やオプション固定など、プロジェクトへの接続口に限定する。外部ライブラリを使ったプロジェクト基盤は `_foundation` に置く。
...
外部依存を持たない純粋 utility のみ置く。DOM / React / `apps/web` 固有設定に依存しない。`Result` 型もここに置く（現状は `_foundation/result.ts`、移設予定）。
...
API モックは MSW を当面採用せず、`fetch` を Vitest でスタブする（`apps/web/src/test/stubAuthFetch.ts`）。テスト実行時は Vitest の `test.env` で `VITE_USE_HTTP_API` を既定で無効化し、認証フロー検証が必要なファイルだけ `vi.stubEnv` で上書きする。

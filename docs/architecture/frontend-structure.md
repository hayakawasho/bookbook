...
- 1つの具象クラスが複数 usecase の port を実装してよい。その対応関係は composition root（`createRepositories`）に閉じ、`Repositories` / `UsecaseDeps` のキーは port（能力）単位を保つ。port 同士を extends で結合したり、既存 port に別 usecase の要求を足したりしない
- endpoint, query string, response schema を閉じ込める
- 外部 response から `_models` への変換は repository 配下の `mappers.ts` に置く
- `_components` には依存しない

1つの具象が複数 port を実装する場合の形（利用側からは常に 1:1 に見える）:

```ts
// _usecases/shelf/ports.ts — 新しい usecase は自分の port を定義する（既存 port に足さない）
export interface ShelfInventoryGateway {
  countByLocation(location: Location): Promise<number>
}

// _repositories/books/httpRepository.ts — 同じ endpoint を担うなら複数 implements
export class HttpBookRepository implements BookRepository, ShelfInventoryGateway { ... }

// _components/app/repositories.ts — 同一インスタンスを port 別のキーで公開
const bookRepo = new HttpBookRepository(client)
return { bookRepo, shelfInventory: bookRepo, ... }
```
